...
def tiktokdl(url) -> bytes:
    ...
    if len(next_data_maybe) != 1:
        raise Exception("Couldn't find a single __NEXT_DATA__ element in the DOM. Are we getting blocked?")

    next_data = json.loads(next_data_maybe[0].string)
    play_addr = next_data["props"]["pageProps"]["itemInfo"]["itemStruct"]["video"]["playAddr"]

    video_headers = {'Accept': 'video/webm,video/ogg,video/*;q=0.9,application/ogg;q=0.7,audio/*;q=0.6,*/*;q=0.5',
    ...
    video = requests.get(play_addr, headers=video_headers, cookies={"tt_webid_v2": tt_webid_v2})
    if not video.ok:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Download MP4 versions of TikTok videos')
    parser.add_argument('url', help='Permalink URL of the TikTok, example: https://www.tiktok.com/@youneszarou/video/6877109293657689345')
